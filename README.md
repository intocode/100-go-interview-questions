...
`defer` откладывает выполнение функции до выхода из текущей функции. Несколько `defer` выполняются в порядке [LIFO](https://ru.wikipedia.org/wiki/LIFO) (последним пришёл — первым ушёл).
...
Пакет `context` координирует горутины: передаёт дедлайны и сигналы отмены, а также значения в рамках запроса. Основное применение — корректное завершение долгих операций (HTTP-запросы, запросы к БД).
...
Map — хеш-таблица. Данные организованы в массив бакетов, каждый из которых хранит до 8 пар ключ-значение. При коллизиях хешей создаются дополнительные бакеты, образующие цепочку.
