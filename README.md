...
Горутина — легковесный поток выполнения, управляемый рантаймом Go, а не операционной системой. Основные отличия:
...
Канал — типобезопасный механизм коммуникации между горутинами.
...
`select` позволяет ждать операции на нескольких каналах одновременно. Выполняется первый готовый case; если готовы несколько — выбирается случайный.
...
### 4. Что произойдёт при записи/чтении в nil-канал и в закрытый канал?
...
`defer` откладывает выполнение функции до выхода из текущей функции. Несколько `defer` выполняются в порядке LIFO (последний добавленный — первый выполненный).

**Важно:** аргументы `defer` вычисляются сразу при объявлении, а не при выполнении:
...
- `sync.Mutex` — эксклюзивная блокировка для чтения и записи
- `sync.RWMutex` — разделяемая блокировка (много читателей или один писатель)
- Каналы — передача данных вместо разделения памяти
- `sync/atomic` — атомарные операции для простых типов

**Обнаружение:** флаг `-race` при сборке или запуске (`go run -race`, `go test -race`).

---

### 7. Для чего используется пакет context и какие основные типы контекстов существуют?

Пакет `context` используется для передачи дедлайнов, сигналов отмены и request-scoped значений между горутинами. Позволяет корректно завершать долгие операции (HTTP-запросы, запросы к БД).
...
- `context.WithValue(parent, key, val)` — передача значений (использовать осторожно)
...
**Work stealing:** если очередь P пуста, M крадёт горутины из очередей других P.

---

### 9. Что такое escape analysis и как понять, что переменная попала в кучу?
...
Слайс — это структура из трёх полей: указатель на underlying array, длина (`len`) и ёмкость (`cap`).
...
- Массив — значение фиксированного размера, копируется целиком при передаче
...
Map — это хеш-таблица с buckets (корзинами). Каждый bucket хранит до 8 пар ключ-значение. При коллизиях элементы добавляются в тот же bucket или в overflow bucket.
...
`panic` — аварийное завершение программы при невосстановимых ошибках. `recover` — перехват паники для graceful shutdown.
...
`sync.WaitGroup` — примитив синхронизации для ожидания завершения группы горутин.
...
Строка в Go — это неизменяемая последовательность байт. Внутри это структура из двух полей: указатель на данные и длина.
...
Embedding (встраивание) — механизм композиции в Go: один тип включает другой без указания имени поля. Встроенный тип «продвигает» свои поля и методы во внешний тип.
...
### 19. Как ограничить количество одновременно работающих горутин?
...
### 22. Что такое дженерики (generics) в Go и какие ограничения у type parameters?

Дженерики (Go 1.18+) — механизм параметризации типов, позволяющий писать функции и структуры, работающие с разными типами данных.
...
Wrapping (Go 1.13+) — добавление контекста к ошибке с сохранением оригинала. Используй `%w` в `fmt.Errorf`:
...
Замыкание — функция, которая захватывает переменные из внешней области видимости. В Go анонимные функции являются замыканиями.
...
Пакет `sync/atomic` предоставляет низкоуровневые атомарные операции над примитивными типами (int32, int64, uint32, uint64, uintptr, unsafe.Pointer).
...
- **Структуры (struct)** — вместо классов, объединяют данные
- **Методы** — функции с receiver'ом, привязаны к типу
- **Embedding** — вместо наследования, композиция типов
- **Интерфейсы** — неявная реализация (duck typing), полиморфизм
...
`iota` — встроенный идентификатор для генерации последовательных целых чисел в блоке констант. Начинается с 0 и увеличивается на 1 для каждой следующей константы.
...
### 37. Что такое zero value в Go и какие типы имеют какие значения по умолчанию?
...
- Указатели, `func`, `interface`, `slice`, `map`, `chan` — `nil`
...
Go имеет встроенную поддержку тестирования через пакет `testing`. Тестовые файлы именуются `*_test.go`, тестовые функции — `func TestXxx(t *testing.T)`.
...
Теги структур — строковые метаданные, прикреплённые к полям структуры. Записываются в обратных кавычках после типа поля.
...
Variadic function — функция, принимающая переменное количество аргументов одного типа. Объявляется с помощью `...` перед типом последнего параметра.
...
`range` — конструкция для итерации по массивам, слайсам, map, строкам и каналам. Возвращает индекс и копию элемента.
...
- **string:** итерация по рунам (не байтам); индекс — позиция байта, значение — `rune`
...
**Решение — использовать `time.NewTimer` с явным сбросом:**
...
**Альтернатива — `time.NewTicker`** для периодических событий.
...
### 54. Что такое пустая структура `struct{}` и когда её использовать?
...
### 58. Можно ли сравнивать структуры в Go и что такое ограничение comparable?
