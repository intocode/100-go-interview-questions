...
1. [Что такое zero value в Go и какие типы имеют какие значения по умолчанию?](#1-что-такое-zero-value-в-go-и-какие-типы-имеют-какие-значения-по-умолчанию)
2. [В каком порядке выполняются defer и когда вычисляются их аргументы?](#2-в-каком-порядке-выполняются-defer-и-когда-вычисляются-их-аргументы)
3. [Как устроен слайс внутри и чем он отличается от массива?](#3-как-устроен-слайс-внутри-и-чем-он-отличается-от-массива)
4. [Как устроена map в Go и что произойдёт при конкурентной записи?](#4-как-устроена-map-в-go-и-что-произойдёт-при-конкурентной-записи)
5. [Когда использовать panic/recover и почему recover работает только в defer?](#5-когда-использовать-panicrecover-и-почему-recover-работает-только-в-defer)
6. [Что такое строка в Go и почему она иммутабельна?](#6-что-такое-строка-в-go-и-почему-она-иммутабельна)
7. [Что такое embedding в Go и чем оно отличается от наследования?](#7-что-такое-embedding-в-go-и-чем-оно-отличается-от-наследования)
8. [Что такое пустая структура `struct{}` и когда её использовать?](#8-что-такое-пустая-структура-struct-и-когда-её-использовать)
9. [В чём разница между pointer receiver и value receiver?](#9-в-чём-разница-между-pointer-receiver-и-value-receiver)
10. [В чём разница между make и new?](#10-в-чём-разница-между-make-и-new)
11. [Можно ли сравнивать структуры в Go и что такое ограничение comparable?](#11-можно-ли-сравнивать-структуры-в-go-и-что-такое-ограничение-comparable)
12. [Как работает wrapping ошибок и зачем нужны errors.Is и errors.As?](#12-как-работает-wrapping-ошибок-и-зачем-нужны-errorsis-и-errorsas)
13. [Что такое замыкание (closure) и какая типичная ошибка связана с циклами?](#13-что-такое-замыкание-closure-и-какая-типичная-ошибка-связана-с-циклами)
14. [Что такое функция init() и в каком порядке она вызывается?](#14-что-такое-функция-init-и-в-каком-порядке-она-вызывается)
15. [Что такое type assertion и type switch? Когда возникает panic?](#15-что-такое-type-assertion-и-type-switch-когда-возникает-panic)
16. [Как реализовано ООП в Go?](#16-как-реализовано-ооп-в-go)
17. [Что такое iota и как она работает?](#17-что-такое-iota-и-как-она-работает)
18. [Что такое теги структур (struct tags) и как они работают?](#18-что-такое-теги-структур-struct-tags-и-как-они-работают)
19. [Что такое variadic functions и как передать слайс в такую функцию?](#19-что-такое-variadic-functions-и-как-передать-слайс-в-такую-функцию)
20. [Что такое build tags (build constraints) и для чего они используются?](#20-что-такое-build-tags-build-constraints-и-для-чего-они-используются)
21. [Как работает range и какие особенности при итерации по разным типам?](#21-как-работает-range-и-какие-особенности-при-итерации-по-разным-типам)
22. [Как работает функция copy для слайсов?](#22-как-работает-функция-copy-для-слайсов)
23. [В чём разница между nil-слайсом и пустым слайсом?](#23-в-чём-разница-между-nil-слайсом-и-пустым-слайсом)
24. [Можно ли вызвать метод на nil-указателе и когда это работает?](#24-можно-ли-вызвать-метод-на-nil-указателе-и-когда-это-работает)
25. [Что такое дженерики (generics) в Go и какие ограничения у type parameters?](#25-что-такое-дженерики-generics-в-go-и-какие-ограничения-у-type-parameters)
26. [Какие типы данных могут быть ключами map в Go?](#26-какие-типы-данных-могут-быть-ключами-map-в-go)
27. [Зачем нужен strings.Builder и почему конкатенация через + неэффективна в цикле?](#27-зачем-нужен-stringsbuilder-и-почему-конкатенация-через-неэффективна-в-цикле)
28. [В чём разница между nil interface и interface с nil-значением внутри?](#28-в-чём-разница-между-nil-interface-и-interface-с-nil-значением-внутри)
29. [Как работают Go Modules и для чего нужны файлы go.mod и go.sum?](#29-как-работают-go-modules-и-для-чего-нужны-файлы-gomod-и-gosum)
30. [Как растёт capacity слайса при append и почему?](#30-как-растёт-capacity-слайса-при-append-и-почему)
31. [Как defer взаимодействует с именованными возвращаемыми значениями?](#31-как-defer-взаимодействует-с-именованными-возвращаемыми-значениями)

**Конкурентность**

32. [Что такое горутина и чем она отличается от потока ОС?](#32-что-такое-горутина-и-чем-она-отличается-от-потока-ос)
33. [Что такое каналы в Go и чем отличаются буферизованные от небуферизованных?](#33-что-такое-каналы-в-go-и-чем-отличаются-буферизованные-от-небуферизованных)
34. [Как работает `select` и что произойдёт, если все каналы заблокированы?](#34-как-работает-select-и-что-произойдёт-если-все-каналы-заблокированы)
35. [Что такое data race и как её избежать в Go?](#35-что-такое-data-race-и-как-её-избежать-в-go)
36. [Для чего используется пакет `context` и какие основные типы контекстов существуют?](#36-для-чего-используется-пакет-context-и-какие-основные-типы-контекстов-существуют)
37. [Что такое sync.WaitGroup и как его правильно использовать?](#37-что-такое-syncwaitgroup-и-как-его-правильно-использовать)
38. [Что такое sync.Once и когда его использовать?](#38-что-такое-synconce-и-когда-его-использовать)
39. [Что такое deadlock и как он возникает в Go?](#39-что-такое-deadlock-и-как-он-возникает-в-go)
40. [Что такое пакет sync/atomic и когда его использовать вместо мьютексов?](#40-что-такое-пакет-syncatomic-и-когда-его-использовать-вместо-мьютексов)
41. [В чём разница между sync.Mutex и sync.RWMutex?](#41-в-чём-разница-между-syncmutex-и-syncrwmutex)
42. [Что такое утечка горутин (goroutine leak) и как её обнаружить?](#42-что-такое-утечка-горутин-goroutine-leak-и-как-её-обнаружить)
43. [Что такое singleflight и когда его использовать?](#43-что-такое-singleflight-и-когда-его-использовать)
44. [Что произойдёт при записи/чтении в nil-канал и в закрытый канал?](#44-что-произойдёт-при-записичтении-в-nil-канал-и-в-закрытый-канал)
45. [Как реализовать graceful shutdown HTTP-сервера в Go?](#45-как-реализовать-graceful-shutdown-http-сервера-в-go)
46. [В чём разница между конкурентностью и параллелизмом?](#46-в-чём-разница-между-конкурентностью-и-параллелизмом)
47. [Как правильно остановить горутину?](#47-как-правильно-остановить-горутину)
48. [Почему нельзя использовать time.After в цикле select и как это приводит к утечке памяти?](#48-почему-нельзя-использовать-timeafter-в-цикле-select-и-как-это-приводит-к-утечке-памяти)
49. [Что такое sync.Pool и когда его использовать?](#49-что-такое-syncpool-и-когда-его-использовать)
50. [Как ограничить количество одновременно работающих горутин?](#50-как-ограничить-количество-одновременно-работающих-горутин)
51. [Что такое sync.Cond и когда его использовать вместо каналов?](#51-что-такое-synccond-и-когда-его-использовать-вместо-каналов)
52. [Что такое errgroup.Group и чем он отличается от sync.WaitGroup?](#52-что-такое-errgroupgroup-и-чем-он-отличается-от-syncwaitgroup)
53. [Что такое паттерны Fan-Out и Fan-In в Go?](#53-что-такое-паттерны-fan-out-и-fan-in-в-go)

**Внутреннее устройство и производительность**

54. [Что такое модель GMP в планировщике Go?](#54-что-такое-модель-gmp-в-планировщике-go)
55. [В чём разница между стеком и кучей в Go и почему аллокация в куче дороже?](#55-в-чём-разница-между-стеком-и-кучей-в-go-и-почему-аллокация-в-куче-дороже)
56. [Как работает сборщик мусора в Go?](#56-как-работает-сборщик-мусора-в-go)
57. [Что такое pprof и как его использовать для профилирования Go-приложения?](#57-что-такое-pprof-и-как-его-использовать-для-профилирования-go-приложения)
58. [Почему порядок итерации по map не детерминирован?](#58-почему-порядок-итерации-по-map-не-детерминирован)
59. [Что такое escape analysis и как понять, что переменная попала в кучу?](#59-что-такое-escape-analysis-и-как-понять-что-переменная-попала-в-кучу)
60. [Что такое рефлексия (reflection) в Go и когда её использовать?](#60-что-такое-рефлексия-reflection-в-go-и-когда-её-использовать)
61. [Как проверить на этапе компиляции, что тип реализует интерфейс?](#61-как-проверить-на-этапе-компиляции-что-тип-реализует-интерфейс)
62. [Как работает тестирование в Go и что такое table-driven tests?](#62-как-работает-тестирование-в-go-и-что-такое-table-driven-tests)
63. [Что такое выравнивание структур (struct alignment) и как оно влияет на размер?](#63-что-такое-выравнивание-структур-struct-alignment-и-как-оно-влияет-на-размер)
64. [Как устроен интерфейс внутри и чем отличается iface от eface?](#64-как-устроен-интерфейс-внутри-и-чем-отличается-iface-от-eface)
65. [Как писать бенчмарки в Go и что такое b.N?](#65-как-писать-бенчмарки-в-go-и-что-такое-bn)

### HTTP и REST API

66. [Как работает http.ListenAndServe и ServeMux внутри?](#66-как-работает-httplistenandserve-и-servemux-внутри)
67. [Что такое идемпотентность HTTP-методов и какие методы являются идемпотентными?](#67-что-такое-идемпотентность-http-методов-и-какие-методы-являются-идемпотентными)
68. [Чем отличаются HTTP-методы PUT и PATCH?](#68-чем-отличаются-http-методы-put-и-patch)
69. [Какие основные HTTP-статус-коды должен знать backend-разработчик?](#69-какие-основные-http-статус-коды-должен-знать-backend-разработчик)
70. [Что такое CORS и зачем он нужен?](#70-что-такое-cors-и-зачем-он-нужен)
71. [Что такое REST и какие принципы REST-архитектуры?](#71-что-такое-rest-и-какие-принципы-rest-архитектуры)
72. [В чём основные отличия HTTP/2 от HTTP/1.1?](#72-в-чём-основные-отличия-http2-от-http11)
73. [Какие способы версионирования REST API существуют и какой предпочтительнее?](#73-какие-способы-версионирования-rest-api-существуют-и-какой-предпочтительнее)
74. [Что такое HATEOAS и почему его редко используют на практике?](#74-что-такое-hateoas-и-почему-его-редко-используют-на-практике)
75. [Что такое Idempotency Key и зачем он нужен в API?](#75-что-такое-idempotency-key-и-зачем-он-нужен-в-api)

### Базы данных

76. [Какие уровни изоляции транзакций существуют в базах данных?](#76-какие-уровни-изоляции-транзакций-существуют-в-базах-данных)
77. [Что такое N+1 проблема в базах данных и как её решить?](#77-что-такое-n1-проблема-в-базах-данных-и-как-её-решить)
78. [Что такое индекс в базе данных и когда его НЕ стоит использовать?](#78-что-такое-индекс-в-базе-данных-и-когда-его-не-стоит-использовать)
79. [Что такое ACID в контексте баз данных?](#79-что-такое-acid-в-контексте-баз-данных)
80. [Что такое connection pooling и зачем он нужен?](#80-что-такое-connection-pooling-и-зачем-он-нужен)
81. [Что такое CAP-теорема и почему нельзя получить все три свойства одновременно?](#81-что-такое-cap-теорема-и-почему-нельзя-получить-все-три-свойства-одновременно)
82. [Чем отличается синхронная репликация от асинхронной?](#82-чем-отличается-синхронная-репликация-от-асинхронной)
83. [Чем отличается шардирование от партиционирования базы данных?](#83-чем-отличается-шардирование-от-партиционирования-базы-данных)
84. [Какие типы JOIN существуют в SQL и чем они отличаются?](#84-какие-типы-join-существуют-в-sql-и-чем-они-отличаются)
85. [Чем отличается оптимистичная блокировка от пессимистичной?](#85-чем-отличается-оптимистичная-блокировка-от-пессимистичной)

### Архитектура и микросервисы

86. [Чем микросервисная архитектура отличается от монолита?](#86-чем-микросервисная-архитектура-отличается-от-монолита)
87. [Что такое Circuit Breaker и зачем он нужен в микросервисах?](#87-что-такое-circuit-breaker-и-зачем-он-нужен-в-микросервисах)
88. [Что такое API Gateway и зачем он нужен в микросервисах?](#88-что-такое-api-gateway-и-зачем-он-нужен-в-микросервисах)
89. [Какие алгоритмы rate limiting существуют и чем они отличаются?](#89-какие-алгоритмы-rate-limiting-существуют-и-чем-они-отличаются)
90. [Что такое Load Balancer и какие алгоритмы балансировки нагрузки существуют?](#90-что-такое-load-balancer-и-какие-алгоритмы-балансировки-нагрузки-существуют)
91. [Что такое gRPC и чем он отличается от REST?](#91-что-такое-grpc-и-чем-он-отличается-от-rest)
92. [Что такое Dead Letter Queue и зачем она нужна?](#92-что-такое-dead-letter-queue-и-зачем-она-нужна)
93. [Что такое reverse proxy и зачем он нужен?](#93-что-такое-reverse-proxy-и-зачем-он-нужен)

### Сети, безопасность и инфраструктура

94. [В чём разница между JWT и сессионной аутентификацией?](#94-в-чём-разница-между-jwt-и-сессионной-аутентификацией)
95. [Чем Docker-контейнер отличается от виртуальной машины?](#95-чем-docker-контейнер-отличается-от-виртуальной-машины)
96. [Что такое multi-stage build в Docker и зачем он нужен для Go-приложений?](#96-что-такое-multi-stage-build-в-docker-и-зачем-он-нужен-для-go-приложений)
97. [Чем отличаются сигналы SIGTERM и SIGKILL в Linux?](#97-чем-отличаются-сигналы-sigterm-и-sigkill-в-linux)
98. [Чем отличается TCP от UDP и что такое TCP handshake?](#98-чем-отличается-tcp-от-udp-и-что-такое-tcp-handshake)
99. [Что происходит, когда вводишь URL в браузер и нажимаешь Enter?](#99-что-происходит-когда-вводишь-url-в-браузер-и-нажимаешь-enter)
100. [Что такое OWASP Top 10 и какие уязвимости туда входят?](#100-что-такое-owasp-top-10-и-какие-уязвимости-туда-входят)
...
### 1. Что такое zero value в Go и какие типы имеют какие значения по умолчанию?
...
### 2. В каком порядке выполняются defer и когда вычисляются их аргументы?
...
### 3. Как устроен слайс внутри и чем он отличается от массива?
...
### 4. Как устроена map в Go и что произойдёт при конкурентной записи?
...
### 5. Когда использовать panic/recover и почему recover работает только в defer?
...
### 6. Что такое строка в Go и почему она иммутабельна?
...
### 7. Что такое embedding в Go и чем оно отличается от наследования?
...
### 8. Что такое пустая структура `struct{}` и когда её использовать?
...
### 9. В чём разница между pointer receiver и value receiver?
...
### 10. В чём разница между make и new?
...
### 11. Можно ли сравнивать структуры в Go и что такое ограничение comparable?
...
### 12. Как работает wrapping ошибок и зачем нужны errors.Is и errors.As?
...
### 13. Что такое замыкание (closure) и какая типичная ошибка связана с циклами?
...
### 14. Что такое функция init() и в каком порядке она вызывается?
...
### 15. Что такое type assertion и type switch? Когда возникает panic?
...
### 17. Что такое iota и как она работает?
...
### 18. Что такое теги структур (struct tags) и как они работают?
...
### 19. Что такое variadic functions и как передать слайс в такую функцию?
...
### 20. Что такое build tags (build constraints) и для чего они используются?
...
### 21. Как работает range и какие особенности при итерации по разным типам?
...
### 22. Как работает функция copy для слайсов?
...
### 23. В чём разница между nil-слайсом и пустым слайсом?
...
### 24. Можно ли вызвать метод на nil-указателе и когда это работает?
...
### 25. Что такое дженерики (generics) в Go и какие ограничения у type parameters?
...
### 26. Какие типы данных могут быть ключами map в Go?
...
### 27. Зачем нужен strings.Builder и почему конкатенация через + неэффективна в цикле?
...
### 28. В чём разница между nil interface и interface с nil-значением внутри?
...
### 29. Как работают Go Modules и для чего нужны файлы go.mod и go.sum?
...
### 30. Как растёт capacity слайса при append и почему?
...
### 31. Как defer взаимодействует с именованными возвращаемыми значениями?
...
### 32. Что такое горутина и чем она отличается от потока ОС?
...
### 33. Что такое каналы в Go и чем отличаются буферизованные от небуферизованных?
...
### 34. Как работает `select` и что произойдёт, если все каналы заблокированы?
...
### 35. Что такое data race и как её избежать в Go?
...
### 36. Для чего используется пакет `context` и какие основные типы контекстов существуют?
...
### 37. Что такое sync.WaitGroup и как его правильно использовать?
...
### 38. Что такое sync.Once и когда его использовать?
...
### 39. Что такое deadlock и как он возникает в Go?
...
### 40. Что такое пакет sync/atomic и когда его использовать вместо мьютексов?
...
### 41. В чём разница между sync.Mutex и sync.RWMutex?
...
### 42. Что такое утечка горутин (goroutine leak) и как её обнаружить?
...
### 43. Что такое singleflight и когда его использовать?
...
### 44. Что произойдёт при записи/чтении в nil-канал и в закрытый канал?
...
### 45. Как реализовать graceful shutdown HTTP-сервера в Go?
...
### 46. В чём разница между конкурентностью и параллелизмом?
...
### 48. Почему нельзя использовать time.After в цикле select и как это приводит к утечке памяти?
...
### 49. Что такое sync.Pool и когда его использовать?
...
### 50. Как ограничить количество одновременно работающих горутин?
...
### 51. Что такое sync.Cond и когда его использовать вместо каналов?
...
### 52. Что такое errgroup.Group и чем он отличается от sync.WaitGroup?
...
### 53. Что такое паттерны Fan-Out и Fan-In в Go?
...
### 54. Что такое модель GMP в планировщике Go?
...
### 55. В чём разница между стеком и кучей в Go и почему аллокация в куче дороже?
...
### 56. Как работает сборщик мусора в Go?
...
### 57. Что такое pprof и как его использовать для профилирования Go-приложения?
...
### 58. Почему порядок итерации по map не детерминирован?
...
### 59. Что такое escape analysis и как понять, что переменная попала в кучу?
...
### 60. Что такое рефлексия (reflection) в Go и когда её использовать?
...
### 61. Как проверить на этапе компиляции, что тип реализует интерфейс?
...
### 62. Как работает тестирование в Go и что такое table-driven tests?
...
### 63. Что такое выравнивание структур (struct alignment) и как оно влияет на размер?
...
### 64. Как устроен интерфейс внутри и чем отличается iface от eface?
...
### 65. Как писать бенчмарки в Go и что такое b.N?
...
### 66. Как работает http.ListenAndServe и ServeMux внутри?
...
### 68. Чем отличаются HTTP-методы PUT и PATCH?
...
### 69. Какие основные HTTP-статус-коды должен знать backend-разработчик?
...
### 70. Что такое CORS и зачем он нужен?
...
### 71. Что такое REST и какие принципы REST-архитектуры?
...
### 72. В чём основные отличия HTTP/2 от HTTP/1.1?
...
### 73. Какие способы версионирования REST API существуют и какой предпочтительнее?
...
### 74. Что такое HATEOAS и почему его редко используют на практике?
...
### 75. Что такое Idempotency Key и зачем он нужен в API?
...
### 76. Какие уровни изоляции транзакций существуют в базах данных?
...
### 77. Что такое N+1 проблема в базах данных и как её решить?
...
### 78. Что такое индекс в базе данных и когда его НЕ стоит использовать?
...
### 79. Что такое ACID в контексте баз данных?
...
### 82. Чем отличается синхронная репликация от асинхронной?
...
### 83. Чем отличается шардирование от партиционирования базы данных?
...
### 84. Какие типы JOIN существуют в SQL и чем они отличаются?
...
### 85. Чем отличается оптимистичная блокировка от пессимистичной?
...
### 86. Чем микросервисная архитектура отличается от монолита?
...
### 87. Что такое Circuit Breaker и зачем он нужен в микросервисах?
...
### 88. Что такое API Gateway и зачем он нужен в микросервисах?
...
### 89. Какие алгоритмы rate limiting существуют и чем они отличаются?
...
### 90. Что такое Load Balancer и какие алгоритмы балансировки нагрузки существуют?
...
### 91. Что такое gRPC и чем он отличается от REST?
...
### 92. Что такое Dead Letter Queue и зачем она нужна?
...
### 93. Что такое reverse proxy и зачем он нужен?
...
### 94. В чём разница между JWT и сессионной аутентификацией?
